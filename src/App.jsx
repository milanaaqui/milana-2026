    ...
    <div className="fade-up">
      ...
      <div style={{ maxWidth: 940, margin: "0 auto", padding: "36px 16px 16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(155px, 1fr))", gap: 8, marginBottom: 28 }}>
          {workerCalcs.map(([ic, ti, de, id, tg]) => (
...
function LegalPage({ t, type }) {
    ...
    return (
      <div className="fade-up" style={{ maxWidth: 700, margin: "0 auto", padding: "24px 16px" }}>
        ...
        <div style={{ background: t.card, padding: 24, borderRadius: 12, border: "1px solid "+t.bd, fontSize: 14, color: t.tx2, lineHeight: 1.6 }}>
          ...
          <p>Cumplimos con la Ley Federal de Protección de Datos Personales en Posesión de los Particulares en México.</p>
        </div>
      </div>
    );
  }
  return (
    <div className="fade-up" style={{ maxWidth: 700, margin: "0 auto", padding: "24px 16px" }}>
      ...
      <div style={{ background: t.card, padding: 24, borderRadius: 12, border: "1px solid "+t.bd, fontSize: 14, color: t.tx2, lineHeight: 1.6 }}>
        <p>MiLana es una plataforma informativa. Los cálculos son estimaciones basadas en la LFT y leyes fiscales de 2026. No sustituyen la asesoría profesional de un contador o abogado.</p>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const goCalc = (c) => { setCalc(c); setPage("calc"); setKey((k) => k + 1); window.scrollTo(0, 0); };
